...
create_split() {
    ...
    local output_dir="${results_root}/splits/${model}/${dataset}/${seed}seed_${shot}shot"

    if [[ ! -f "${npz_path}" ]]; then
        echo "Missing clean prediction npz for split creation: ${npz_path}" >&2
        exit 1
    fi

    if [[ "${skip_existing}" == "1" && -f "${output_dir}/all.csv" ]]; then
        echo "==> Skip existing split: model=${model}, dataset=${dataset}"
        return
    fi

    echo "==> Create split: model=${model}, dataset=${dataset}"
...
for model in ${models}; do
    for dataset in ${datasets}; do
        ...
        create_split "${model}" "${dataset}"
    done
done

for model in ${models}; do
    for dataset in ${datasets}; do
        split_dir="${results_root}/splits/${model}/${dataset}/${seed}seed_${shot}shot"
