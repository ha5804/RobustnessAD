...
def local_map_from_patches(patch_features, text_features, image_size, temperature):
    maps = []
...
@torch.no_grad()
def predict_batch(model, images, cls_names, text_cache, args, device):
    ...
    if args.object_agnostic or len(set(cls_names)) == 1:
        ...
        pixel_maps = local_map_from_patches(patch_features, text_features, args.image_size, args.temperature)
    ...
    for idx, cls_name in enumerate(cls_names):
        ...
        pixel_maps.append(local_map_from_patches(one_patch_features, text_features, args.image_size, args.temperature))
...
def test(args):
    ...
    model, _ = anomalycliplib.load(args.pretrained_model, device=device)
    if hasattr(model.visual, "DAPM_replace") and args.dpam_layer is not None:
        model.visual.DAPM_replace(DPAM_layer=args.dpam_layer)
    load_optional_checkpoint(model, args.checkpoint_path, logger)
    model.to(device)
    model.eval()
    ...
    for items in tqdm(test_loader, desc="Testing"):
        ...
        image_anomaly_pred, pixel_anomaly_map = predict_batch(model, query_image, batch_cls_names, text_cache, args, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_selected_heatmaps", action=argparse.BooleanOptionalAction, default=True)
